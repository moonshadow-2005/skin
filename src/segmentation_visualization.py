...
from src.project_paths import DEFAULT_MODEL_PATH, LABELED_DATASET_DIR, RESULTS_DIR, resolve_model_path
...
def visualize_prediction(image_path, model_path=DEFAULT_MODEL_PATH, output_dir=RESULTS_DIR, device='cuda'):
    """
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        try:
            num = int(sys.argv[1])
            # 构建图片路径
            image_path = str(LABELED_DATASET_DIR / f"{num}.jpg")
