...
from src.project_paths import ORIENTATION_OUTPUT_DIR, REPORT_OUTPUT_DIR, TEXTURE_OUTPUT_DIR
...
def calculate_outward_direction(angles, sector_center_angle):
    ...
    # 计算0-180度范围内的主方向
    from src.orientation_analysis import calculate_circular_mean_degrees
...
def plot_radial_sectors(img, orientations, mask, output_dir, step=20):
    ...
    # === 保存扇区信息供报告模块使用 ===
    from src.legacy_report import save_sector_info
...
if __name__ == "__main__":
    ...
    input_path = TEXTURE_OUTPUT_DIR / f"only_texture_line_{num}.png"

    if not input_path.exists():
        input_path = TEXTURE_OUTPUT_DIR / f"only_texture_line_{num}.jpg"
    ...
    analyze_texture_orientation(str(input_path))
