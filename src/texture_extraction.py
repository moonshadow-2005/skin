...
def analyze_skin_texture(image_path, model_path=DEFAULT_MODEL_PATH, device='cuda'):
    """
    皮肤纹理分析主函数
    ...
    line_only_path = str(output_dir / f"only_texture_line_{base_name}.png")
    ...
    # 纯纹理线条图
    axes[1].set_title("Texture Lines")
    axes[1].imshow(texture_lines, cmap='gray')
    axes[1].axis('off')

    # 叠加显示（仅在受影响区域显示）
    axes[2].set_title("Texture Line Overlay")

    # Generate a red texture layer from the same refined binary lines used by
    # downstream analysis. Black pixels remain fully transparent.
    line_color = np.zeros_like(original_img)
    line_mask = refined_lines > 0
    line_color[line_mask] = [255, 0, 0]

    overlay = original_img.copy()
    blended = cv2.addWeighted(original_img, 0.70, line_color, 0.30, 0)
    overlay[line_mask] = blended[line_mask]

    # Save the overlay panel separately for the Demo process views.
    overlay_path = str(output_dir / f"texture_overlay_{base_name}.png")
    overlay_bgr = cv2.cvtColor(overlay, cv2.COLOR_RGB2BGR)
    ok, encoded = cv2.imencode('.png', overlay_bgr)
    if not ok:
        raise RuntimeError(f"纹理叠加图编码失败: {overlay_path}")
    encoded.tofile(overlay_path)

    axes[2].imshow(overlay)
    axes[2].axis('off')

    # 修改保存路径
    output_path = str(output_dir / f"texture_line_{base_name}.png")
    ...
    print(f"纹理叠加图已保存至: {overlay_path}")
