...
from src.project_paths import LABELED_DATASET_DIR, MODELS_DIR, TRAINING_OUTPUT_DIR
...
# 训练函数
def train_model(model, train_loader, val_loader, criterion, optimizer, num_epochs=10, device='cuda', model_name=MODELS_DIR / 'best_unet_model.pth', patience=7):
    model.to(device)
...
def main():
    ...
    if use_transform:
        model_name = str(MODELS_DIR / f'best_trans_unet_model_{timestamp_str}.pth')
        final_model_name = str(MODELS_DIR / f'final_trans_unet_model_{timestamp_str}.pth')
        history_file = str(TRAINING_OUTPUT_DIR / f'training_history_trans_{timestamp_str}.json')
        print(f'数据增强模式: 启用3倍数据增强 (原图 + 两种颜色抖动)')
    else:
        model_name = str(MODELS_DIR / f'best_unet_model_{timestamp_str}.pth')
        final_model_name = str(MODELS_DIR / f'final_unet_model_{timestamp_str}.pth')
        history_file = str(TRAINING_OUTPUT_DIR / f'training_history_{timestamp_str}.json')
    ...
    data_dir = str(LABELED_DATASET_DIR)
