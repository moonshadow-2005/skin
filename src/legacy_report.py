...
from src.project_paths import ORIENTATION_OUTPUT_DIR, REPORT_OUTPUT_DIR
...
def generate_summary_report(sector_info, num):
    ...
    # 从历史方向分析产物复制 spatial_sector_directions_{编号}.png 作为概览图。
    source_overview = str(ORIENTATION_OUTPUT_DIR / f'spatial_sector_directions_{num}.png')
...
def save_sector_info(sector_info, num):
    """保存扇区信息到文件，供报告模块使用"""
    output_dir = str(ORIENTATION_OUTPUT_DIR)
    ...
    print(f"扇区信息已保存至: {ORIENTATION_OUTPUT_DIR / f'sector_info_{num}.pkl'}")

def load_sector_info(num):
    """从文件加载扇区信息"""
    pickle_path = str(ORIENTATION_OUTPUT_DIR / f'sector_info_{num}.pkl')
...
def generate_report(num):
    ...
    try:
        ...
        print(f"报告文件: {REPORT_OUTPUT_DIR / f'{num}_skin_texture_analysis_report.md'}")
        print(f"相关图片: {REPORT_OUTPUT_DIR / f'{num}_pic'}")
...
if __name__ == "__main__":
    ...
    # 检查扇区信息文件是否存在
    sector_info_path = str(ORIENTATION_OUTPUT_DIR / f'sector_info_{num}.pkl')
    if not os.path.exists(sector_info_path):
        ...
        print(f"3. {ORIENTATION_OUTPUT_DIR} 目录存在且包含相关文件")
        sys.exit(1)
    ...
    try:
        ...
        print(f"\n=== 报告生成完成 ===")
        print(f"您可以查看以下文件：")
        print(f"• 主报告：{REPORT_OUTPUT_DIR / f'{num}_skin_texture_analysis_report.md'}")
        print(f"• 图片文件夹：{REPORT_OUTPUT_DIR / f'{num}_pic'}")
